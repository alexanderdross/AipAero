"""Turkmenistan (Turkmenhowayollary (AIP hosted by the Russian CAICA)) info-page crawler.

Turkmenistan's official AIP is published by Turkmenhowayollary (AIP hosted by the Russian CAICA). However the AIP is a CAICA JavaScript "DHTML-menu" frameset (no static AD-2 / chart-PDF links) - and we
deliberately do NOT run a browser to reverse-engineer it, so Turkmenistan is onboarded
...
The aerodrome LIST comes from OurAirports (public domain / CC0): the Turkmenistan
(`iso_country = TM`) aerodromes with a real ICAO (UT** (UTA*)), emitted as `vfr` rows
...
# Official AIP portal. The website flags TM as a gated country so the detail
...
class TM(HttpCrawlerBase):
    ...
    def crawl(self) -> list[Airport]:
        ...
        try:
            ...
            self.logger.error(f"TM crawl failed: {e}")
            raise
        finally:
            self.close()
