...
- **`hours_structured` (TEXT, JSON)** - 7 entries (index 0 = Monday .. 6 = Sunday), each `{ open, close, flag }`:
  - `open` / `close` are **minutes after UTC midnight** (0..1439), or `null` for sun-relative / unknown days. AIP AD 2.3 is published in UTC (ICAO Annex 15), and the site displays UTC throughout, so `openStatus`/`isOpenUntil` compute in UTC and the UI labels times `Z` / "UTC". Daylight saving is handled at the override layer (winter/summer UTC pairs, see "Timezone" below), not by a local-time display. (This replaced an earlier longitude-based local-time approximation, and then a short-lived LT-override display - both wrong or confusing for a UTC source.)
...
### Timezone: UTC (Zulu) everywhere; daylight saving via seasonal override windows

AIP AD 2.3 hours are published in **UTC** (ICAO Annex 15), pilots plan in UTC, and the site displays **UTC everywhere** - the map "open until 19:00" input is 19:00 UTC (labelled "UTC" next to the picker), the detail-page badge marks its times with `Z` ("closes 19:00Z"), the weekday table is labelled "(UTC)", and the JSON-LD carries UTC numbers. `openStatus`/`isOpenUntil` compute the day-of-week and minute-of-day in UTC. There is **no local-time display path** (a short-lived `LT` override display, 20.07.2026, was reverted the same day: showing UTC sun times next to LT hours read as an inconsistent mix - owner decision "alle Zeiten UTC").

**Daylight saving (owner decision, 20.07.2026).** The common `0500-2100 (0400-2000)` AIP form is a UTC pair - winter primary, summer in parentheses (bracket = summer period) - i.e. the same field opens an hour earlier in UTC during summer time. The AIP marks this with a footnote (`0730-1600 (0630-1500)`, or `-1 HR during summer time`); we keep the base UTC value and **explain the convention in the glossary** (`GlossaryPage.terms.operatinghours`) rather than converting to local time. To keep the badge correct year-round without an LT display, a verified override stores BOTH seasons' UTC windows (`{ winter, summer?, tz }` in `hours-overrides.ts`) and `resolveOverrideHours(icao, when)` returns the window for the season active now - `isDstActive(tz, when)` (an `Intl` offset comparison in `opening-hours.ts`) picks summer vs winter. The `tz` is used ONLY for that seasonal pick; the returned window is plain UTC, so nothing displays a local clock. Automatic (eAIP/OCR) sources keep a single UTC window (the summer bracket is discarded at ingest, see "Out of scope"); the glossary note covers their seasonal shift.
...
### Global OCR fallback for image-only PDF eAIPs (21.07.2026)

DE aside, six crawlers (ES/GR/RO/MK/RS/DK) read AD 2.3 hours from a PDF via `HttpCrawlerBase.pdf_text` -> `ad23_hours`. A scanned/image-only PDF has no text layer, so pypdf returned `""` and those fields got no hours. `pdf_text` now falls back to OCR: when the extracted text is effectively empty (`< 50` chars) it rasterises up to `MAX_OCR_PAGES` (AD 2.3 sits early) with **pypdfium2** (permissive licence, pure wheel with bundled PDFium - no poppler, no AGPL) and OCRs each page via `de_ocr.ocr_pil(img, PDF_OCR_LANG)`. `PDF_OCR_LANG` is per crawler (GR `ell+eng`, RO `ron+eng`, MK `mkd+eng`, ES `spa+eng`, RS `hrv+eng`, DK `dan+eng`; base `eng` since the AD 2.3 parse targets English tokens).

Provenance (safety): `pdf_text` records `_last_pdf_ocr`; each caller stamps `hours_source_by_icao[icao] = "pdf-ocr-hours"` for an OCR'd field, and `main.py` passes `publish_hours(source_by_icao=...)` so ONLY the OCR'd fields carry that source while clean-text siblings stay `eaip`. Website-side `"pdf-ocr-hours"` joins the API enum, shares `hoursRank` **3** with `dfs-ocr-hours` (AIP-derived OCR: above community, below clean `eaip`), and lights up the same always-visible `hoursOcrDisclaimer`. Tesseract + language packs install on every crawl now (`crawl.yml`), not only under `de_ocr`. Fully fail-soft: a missing rasteriser/Tesseract, or an image AD 2.3 with no English tokens, simply yields no hours - never a wrong claim. Unit-tested with the rasteriser + OCR mocked (`tests/test_pdf_ocr.py`, `tests/test_output_handler.py`); real OCR quality is validated live via `crawler-live-test.yml` (MK's ~2018 image PDFs are the expected first beneficiary).

### DK (Naviair "4. Operational hours" data sheet) + AT (charts-only) (21.07.2026)

A live-test showed **DK 0/34** and **AT 0/72** despite both being wired. Four recon rounds pinned the causes (both the safe failure - no hours shown, never a wrong time):

- **DK is a labelling mismatch, now fixed (30/34 live).** Naviair's `EK_AD_2_<ICAO>_en.pdf` data sheet (the "01. AD 2 &lt;ICAO&gt; text" doc) has a clean text layer but does NOT use the ICAO `AD 2.3` heading `ad23_hours` keys on - it is a flat, bilingual **"4. Operational hours"** layout where the aerodrome window is the **`AD:`** row (APP/TWR/ADO/ARO/AIS/MET are H24 service rows, not the field's hours). `crawlers/crawlers/dk_hours.py` (`parse_dk_hours`) isolates the English item-4 region, takes the `AD:` row, drops the parenthetical summer window + PPR notes, splits into per-day statements, and reuses the shared `parse_ad23_text` (clean text -> source `eaip`). It also taught `parse_ad23_text` to read a literal `CLSD`/`CLOSED` day (e.g. `SAT-SUN CLSD` -> `{"kind":"closed"}`, only from the token). Verified against EKKA/Karup (`MON-FRI 0500-1700Z, SAT-SUN CLSD`).
- **AT is charts-only - hours not available.** Austro Control publishes each aerodrome's AD 2 data as chart PDFs; the per-field `ad_2_<icao>.htm` page is a chart-index table with no AD 2.3 text, and none of a field's chart PDFs carry the hours table (checked all 15 of LOWG's). The futile per-field `ad23_hours` fetch (72 wasted requests/crawl) was removed; **AT operating hours come from OpenAIP** on the website. (Same recon renamed the AT chart box entries to the AD-2 table's right-column descriptions, e.g. "Aerodrome Chart - ICAO".)

**DK dual-source (21.07.2026).** Naviair publishes DK aerodromes in TWO products: `02. VFR Flight Guide` (the historic source) and `01. AIP Danmark` (the full/IFR AIP, which carries the IFR charts the VFG omits). `dk.py` now walks BOTH AD-2 trees and merges by ICAO, unioning each field's charts deduped by FILENAME (both publish shared charts as separate media files with identical filenames, so basename-dedup collapses those while keeping each product's unique sheets). Live: the AIP's 13 aerodromes are all a subset of the VFG's 32, so the field count is unchanged (34), but the chart total jumped **~133 -> 330** (the AIP's IAC/SID/STAR/GMC/AOC sheets on top of the VFG's VFR set); hours stay 30/34 (parsed per source, so AIP-only fields would gain hours too). AD 4 PRIVATE is now attempted (fail-soft; still 0 per-field entries live).
...
Each entry stores `{ winter, summer?, tz }` - the winter (base) and optional summer **UTC** windows plus the field's IANA zone. `resolveOverrideHours(icao, when)` returns the season active at `when` (`isDstActive(tz)` picks summer vs winter) as plain UTC `StructuredHours`, so the badge/table/JSON-LD and the map filter all show the season currently in effect, in UTC, labelled `Z`/"(UTC)" (updated 20.07.2026 from a brief LOCAL-time seed that was reverted the same day - owner wants all UTC, with DST explained in the glossary). The `tz` is used ONLY for the seasonal pick, never to display a local clock. Solar limits stay the `ss`/`sr` boundary, resolved to the field's real sunset/sunrise UTC. The write-back never persists an override's window into D1 (it is a season snapshot; a later read in the other season would serve the wrong one). Seeded with **EDNY** (`Europe/Berlin`: winter Mon-Fri 05:00-21:00Z / Sat-Sun 08:00Z-SS, summer 04:00-20:00Z / 07:00Z-SS).
...
- `pnpm check` + `pnpm test` (vitest) - `opening-hours.ts`: fixed windows, SR/SS resolution at a known lat/lon/date, H24, NOTAM->unknown, `isOpenUntil` boundary at exactly 19:00, **and `isDstActive`** (Europe/Berlin false in Jan / true in Jul; a no-DST zone always false); `hours-overrides.ts`: `resolveOverrideHours` returns the winter UTC window in January and the summer one in July, and the EDNY badge is season-correct in UTC (open at 20:30Z in winter, closed at 20:30Z in summer).
...
- Post-deploy - `node scripts/verify-hours.mjs [--base-url=https://aip.aero]` tolerantly parses the live EDNY DE detail page and asserts the active-season **UTC** window (JSON-LD `openingHoursSpecification` = 05:00-21:00 in winter / 04:00-20:00 in summer, chosen by the runner's current DST state), the "(UTC)" weekday-table label, no leaked `LT` frame, the authoritative badge label, and the ABSENCE of the OCR disclaimer. Not a CI test - the Playwright suite runs against `next start` with an absent D1, so `?ICAO` detail rows do not exist there.
