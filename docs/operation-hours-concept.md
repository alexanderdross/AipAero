...
The OCR can be wrong (e.g. EDNY's weekend `SS` read as `$8`, dropping the SAT/SUN window). `hours-overrides.ts` mirrors `customs-overrides.ts`: a per-ICAO map of VERIFIED `StructuredHours` (from the field's AIP AD 2.3) that WINS over every automatic source at read time - in `getAirportFacts` (source becomes `eaip`, so the badge reads "official" and the OCR disclaimer drops) AND in the airport-coords API (so the map filter agrees). Times are **UTC** (the site shows UTC/Zulu throughout); a winter/summer pair is stored as the non-bracketed winter value and the <= 1 h DST drift a single UTC window cannot express is covered by the always-visible advisory note (the owner declined a full local-time model). Solar limits are stored as the `ss`/`sr` boundary, which `openStatus` resolves to the field's real sunset/sunrise UTC (that half tracks the season exactly). Seeded with **EDNY** (Mon-Fri 05:00-21:00Z, Sat/Sun 08:00Z-SS). Add entries ONLY from a verified AIP AD 2.3 - a wrong hours claim is a safety hazard.
