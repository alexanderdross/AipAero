...
export function generateStaticParams() {
  return routing.locales
    .filter((locale) => countryHasType(localeCountryMapping[locale]!, "ifr"))
    .map((locale) => ({ locale }));
}
    ...
    alternates: {
      ...
      languages: isSingleLocale(locale)
        ...
        : {
            ...
            "x-default":
              new URL(
                getPathname({ href: "/ifr", locale: englishLocale }),
                orgUrl,
              ).toString() + `${data ? `?${data.slug}` : ""}`,
          },
    },
...
export default async function IndexPage({
  ...
  if (data) {
    currentUrl = new URL(
      getPathname({
        href: { pathname: "/ifr", query: { [data.slug]: "" } },
        locale,
      }),
      orgUrl,
    )
