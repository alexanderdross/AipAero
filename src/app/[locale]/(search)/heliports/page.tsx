...
export function generateStaticParams() {
  return routing.locales
    .filter((locale) =>
      countryHasType(localeCountryMapping[locale]!, "heliport"),
    )
    .map((locale) => ({ locale }));
}
...
export async function generateMetadata(
  ...
  if (p.at(0) !== undefined) {
    data = await QUERIES.airport(p.at(0)!, country, "heliport");
    if (!data) {
      return notFound();
    }
        ...
        : {
            ...Object.assign(
              ...
              ...locales.map((l) => ({
                [localeLangMapping[l]!]:
                  new URL(
                    getPathname({ href: "/heliports", locale: l }),
                    orgUrl,
            ...
            "x-default":
              new URL(
                getPathname({ href: "/heliports", locale: englishLocale }),
                orgUrl,
              ).toString() + `${data ? `?${data.slug}` : ""}`,
          },
    },
...
export default async function IndexPage({
  ...
  let currentUrl = new URL(
    getPathname({ href: "/heliports", locale }),
    orgUrl,
  ).toString();
  ...
  if (data) {
    currentUrl = new URL(
      getPathname({
        href: { pathname: "/heliports", query: { [data.slug]: "" } },
        locale,
      }),
      orgUrl,
    )
