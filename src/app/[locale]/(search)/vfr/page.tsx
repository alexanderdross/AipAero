...
export async function generateMetadata(
  ...
  if (p.at(0) !== undefined) {
    data = await QUERIES.airport(p.at(0)!, country, "vfr");
    if (!data) {
      return notFound();
    }
    ...
    alternates: {
      ...
      languages: isSingleLocale(locale)
        ...
        : {
            ...
            "x-default":
              new URL(
                getPathname({ href: "/vfr", locale: englishLocale }),
                orgUrl,
              ).toString() + `${data ? `?${data.slug}` : ""}`,
          },
    },
...
export default async function IndexPage({
  ...
  if (data) {
    currentUrl = new URL(
      getPathname({
        href: { pathname: "/vfr", query: { [data.slug]: "" } },
        locale,
      }),
      orgUrl,
    )
