...
import { getPathname, localeCountryMapping, localeLangMapping } from '~/i18n/routing';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'MilitaryPage' });
  const parentMetadata = await parent;
  const previousOpenGraph = parentMetadata.openGraph ?? {};
  const previousOther = parentMetadata.other ?? {};
  const pathname = getPathname({ href: '/military', locale });
  ...
  if (p.at(0) !== undefined) {
    data = await QUERIES.airport(p.at(0) as string, country, 'mil');
    if (!data) {
      return notFound();
    }
  ...
  return {
    ...
    alternates: {
      ...
      languages: locale === 'uk' ? undefined : Object.assign({}, ...locales.map((l) => ({
        [localeLangMapping[l] as string]: new URL(getPathname({ href: '/military', locale: l }), orgUrl).toString() + `${data ? `?${data.slug}` : ''}`
      })))
    },
    openGraph: {
      ...previousOpenGraph,
...
export default async function IndexPage({
  ...
  const p = Object.keys((await searchParams));
  const t = await getTranslations('MilitaryPage');
  ...
  let currentUrl = new URL(getPathname({ href: '/military', locale }), orgUrl).toString();
  ...
  const breadcrumbsSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 3,
        "item": {
          "@id": new URL(getPathname({ href: '/military', locale }), orgUrl).toString(),
  ...
  if (data) {
    currentUrl = new URL(getPathname({ href: { pathname: '/military', query: { [data.slug]: '' } }, locale }), orgUrl).toString().replace('=', '');
    schemaProductName = t('resultTitle', { airport: data.title });
    schemaAlternateName = data.icao ? `AIP MIL ${data.icao}` : data.title;
