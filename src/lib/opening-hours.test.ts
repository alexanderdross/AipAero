...
describe("isDstActive (season selection for UTC overrides)", () => {
  it("Europe/Berlin: false in January, true in July", () => {
    expect(isDstActive("Europe/Berlin", new Date("2026-01-14T12:00:00Z"))).toBe(
      false,
    );
    expect(isDstActive("Europe/Berlin", new Date("2026-07-14T12:00:00Z"))).toBe(
      true,
    );
  });

  it("a no-DST zone is always false (winter/base window applies)", () => {
    // UTC and a zone with no daylight saving never report DST active.
    expect(isDstActive("UTC", new Date("2026-07-14T12:00:00Z"))).toBe(false);
    expect(isDstActive("Etc/GMT-4", new Date("2026-07-14T12:00:00Z"))).toBe(
      false,
    );
  });
});
