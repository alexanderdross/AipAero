...
- **PWA** - `src/app/manifest.ts` emits `/manifest.webmanifest` so the site is installable (e.g. on an EFB tablet). No service worker / offline support yet - the concept for that (hand-rolled SW, per-resource cache strategies, explicit "save chart for offline", AIRAC-staleness banners) is specced in `docs/pwa-offline-concept.md`.
