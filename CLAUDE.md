...
- **Every active crawler is import-checked in CI** (`compileall` over the whole `crawlers` package, a representative-subset import smoke test, plus the `pytest` suite - the OurAirports info-page crawlers additionally by `test_infopage_integrity.py`). They run on httpx/BeautifulSoup (DK walks the Naviair Umbraco JSON API with httpx; its Playwright render remains only as a self-diagnosing fallback); Selenium is gone - the legacy `crawler_base.py` / `eurocontrol_base.py` bases, the experimental crawlers (belgium / car_sam_nam / pac_n / pac_p / run) and the `cache_warmer.py` script were removed along with the `selenium` / `webdriver-manager` deps.
...
1. **Python crawlers** scrape AIP websites for airport data. All fifty active country crawlers run on `httpx` + BeautifulSoup (DK and RS use Playwright for their JS-rendered sources). The crawler subsystem also retries transient HTTP failures with exponential backoff, and `OutputHandler` refuses to publish if the new airport count drops > 50% from the last successful run (override with `CRAWLER_FORCE_PUBLISH=1`).
...
> The three reference tables below (Locale Mappings, Page Availability, Localized Pathnames) list the **founding five** countries as worked examples. The **live source of truth** is code, not this doc: locales/prefixes/slugs in `src/i18n/routing.ts`, per-country page types in `countryTypeAvailability`, and which countries are launched in `liveCountries` (all in `src/lib/utils.ts`). Newer countries (BE, CZ, DK, GR, NO, PL, SE, EE, FI, ES, LV, IS, PT, HU, SI, LT, RS, IE, SK, BA, CH, AL, MK, RO, CY, MT, MD, IT, HR, BG, TR, GE, AM, AZ, UA, UZ, BY, KZ, XK, RU, TJ, TM, KG, AU, NZ) follow the same shape - e.g. LT: locales `lt`/`lt-EN`, prefix `/lt/en`, VFR only, slug `/oro-uostu-sarasas-lietuva`; RS: locales `rs`/`rs-EN`, prefix `/rs/en`, VFR only, slug `/lista-aerodroma-srbija`; IE: single English locale `ie` (like uk/be), VFR only, slug `/airport-list-ireland`; SK: locales `sk`/`sk-EN`, prefix `/sk/en`, VFR only, slug `/letiska-slovensko`; BA: locales `ba`/`ba-EN`, prefix `/ba/en`, VFR only, slug `/lista-aerodroma-bih`; CH: locales `ch`/`ch-EN` (German native), prefix `/ch/en`, VFR only, slug `/flugplaetze-schweiz`, an **info-page** (gated - links the skybriefing portal, no chart crawl); AL: locales `al`/`al-EN` (Albanian native), prefix `/al/en`, VFR only, slug `/aeroportet-shqiperi`; MK: locales `mk`/`mk-EN` (Macedonian native), prefix `/mk/en`, VFR only, slug `/aerodromi-severna-makedonija` (M-NAV open eAIP, 2 international fields); RO: locales `ro`/`ro-EN` (Romanian native), prefix `/ro/en`, VFR only, slug `/lista-aeroporturi-romania` (AISRO static DOCS tree, 35 fields, browser headers); CY: locales `cy`/`cy-EN` (Greek native), prefix `/cy/en`, VFR only, slug `/aerolimenes-kypros` (DCA "Open Cyprus VFR Manual", a bespoke static frameset - `cy.py` reads `menu.html` for each aerodrome's `charts/VFR_CHART_<ICAO>.pdf`; 2 civil fields LCLK/LCPH, 100% chart coverage; NOT gated); MT: single English locale `mt` (like uk/ie/be, bilingual but English-official), VFR only, slug `/airport-list-malta`, an **info-page** (gated - the MATS AIP portal is a JS app, links Transport Malta's AIP page, no chart crawl); MD: locales `md`/`md-EN` (Romanian native, Moldovan = Romanian), prefix `/md/en`, VFR only, slug `/lista-aeroporturi-moldova`, an **info-page** (gated - the MOLDATSA `aim.moldatsa.md` portal is Home-Briefing-registration-gated, no chart crawl); AU: single English locale `au` (like uk/ie/mt), VFR only, slug `/airport-list-australia`, an **info-page** (gated - the Airservices DAP charts are a paid subscription, links the Airservices AIP portal, no chart crawl); NZ: single English locale `nz` (like uk/ie/mt), VFR only, slug `/airport-list-new-zealand`, an **info-page** (gated - aip.net.nz is an Incapsula-WAF WebForms portal with no static chart tree, links the portal, no chart crawl).
...
**Legal pages are ROOT-level, single-language pages paired by topic OUTSIDE `[locale]`** (owner decision, 18.07.2026): English at the root `/terms`, `/imprint`, `/privacy`; German served under `/de/agb`, `/de/impressum`, `/de/datenschutz` (native slugs German users actually search). One clean canonical URL per language, NOT localized into every site locale - they are legal/compliance pages, not SEO landing pages. Each pair is cross-linked with a language toggle + `hreflang` alternates (`en`/`de`/`x-default`->English). **The German pages are the SAME root routes** (`src/app/{impressum,datenschutz,agb}`), served under `/de/` via a `middleware.ts` **rewrite** (`/de/impressum` -> the root route, URL unchanged) + a 301 of the bare `/impressum` -> `/de/impressum`: they can NOT be real `[locale]` routes, because the site header's language switcher would then link to a non-existent `/de/en/impressum` twin. Their canonical + `hreflang` `de` point at `/de/...`; the English page's `hreflang` `de` and its toggle point at `/de/...` too. They live at `src/app/{terms,agb,imprint,impressum,privacy,datenschutz}/page.tsx`, render their own `<html>`/`<body>` + Header + `<Footer global />` via the shared `src/components/legal-shell.tsx`. The shell's `lang` prop drives both `<html lang>` AND the site-chrome locale (`setRequestLocale`): English pages pin `"uk"`, German pages `"de"` (so a German legal page gets a German header/footer). Pinning the locale also keeps them STATIC / prerendered (otherwise next-intl falls back to `headers()` -> dynamic render, and on Workers only prerendered pages reliably carry their `<head>` metadata). Content is hardcoded single-language JSX - so they are NOT in `routing.pathnames`, NOT in the i18n message files, and NOT in the per-country sitemap. Provider data (VAT/tax/contact) + the privacy service links + the `AIP_SOURCES` cc-keyed attribution (rendered on BOTH terms pages) live in `src/lib/legal.ts` (drift-guarded by `check-live-countries-coverage.mjs`, which reads `src/lib/legal.ts`). The old `/<locale>/terms` URLs 301-redirect to `/terms` in `middleware.ts` (regex `^/[a-z]{2}(?:/en)?/(terms|imprint|privacy)/?$`). The footer's legal links point at the fixed root paths (German slugs on the `de`/`at`/`ch` locales, English otherwise) with their existing localized labels (`Footer.{terms,imprint,privacy}.title`). Contact e-mail (`mail@dross.net`) is shown but hyperlinked to `dross.net/contact`; the responsible person's name links to `dross.net/alexander/`.
...
The table below is sorted by country name. Info-page countries (gated login/paywall/WAF portals - no chart crawl) read their aerodrome list from OurAirports and link the official portal; they are in `gatedCountries` in `src/lib/utils.ts`. The live source of truth for the roster is `COUNTRY_CRAWLERS` in `crawlers/main.py`.

| Country              | Code | Crawler Class | Base                    | Browser?                                         | AIP Source                                 |
| -------------------- | ---- | ------------- | ----------------------- | ------------------------------------------------ | ------------------------------------------ |
| Albania              | AL   | `AL`          | `HttpEurocontrolBase`   | no                                               | Albcontrol eAIP                            |
| Armenia              | AM   | `AM`          | `HttpEurocontrolBase`   | no                                               | ARMATS eAIP (armats.am)                    |
| Australia            | AU   | `AU`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | Airservices Australia (paid DAP, gated)    |
| Austria              | AT   | `AT`          | `HttpCrawlerBase`       | no                                               | Austro Control eAIP                        |
| Azerbaijan           | AZ   | `AZ`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | no open live AIP (gated)                   |
| Belarus              | BY   | `BY`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | ban.by (registration-gated)                |
| Belgium/Luxembourg   | BE   | `BE`          | `HttpEurocontrolBase`   | no                                               | skeyes eAIP                                |
| Bosnia & Herzegovina | BA   | `BA`          | `HttpEurocontrolBase`   | no                                               | BHANSA eAIP                                |
| Bulgaria             | BG   | `BG`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | BULATSA b-flip (registration-gated)        |
| Croatia              | HR   | `HR`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | Croatia Control AIM Portal (subscription)  |
| Cyprus               | CY   | `CY`          | `HttpCrawlerBase`       | no (static VFR-manual tree)                      | DCA "Open Cyprus VFR Manual"               |
| Czechia              | CZ   | `CZ`          | `HttpEurocontrolBase`   | no                                               | ANS CR (rlp.cz) eAIP + VFR Manual          |
| Denmark              | DK   | `DK`          | `PlaywrightCrawlerBase` | no (JSON API; render = diagnostic fallback only) | Naviair Umbraco JSON API                   |
| Estonia              | EE   | `EE`          | `HttpEurocontrolBase`   | no                                               | EANS (Estonia) eAIP                        |
| Finland              | FI   | `FI`          | `HttpEurocontrolBase`   | no                                               | Fintraffic eAIP                            |
| France               | FR   | `FR`          | `HttpEurocontrolBase`   | no                                               | SIA eAIP                                   |
| Georgia              | GE   | `GE`          | `HttpEurocontrolBase`   | no                                               | Sakaeronavigatsia eAIP (airnav.ge)         |
...
| Hungary              | HU   | `HU`          | `HttpEurocontrolBase`   | no                                               | HungaroControl eAIP + VFR Manual           |
| Iceland              | IS   | `IS`          | `HttpEurocontrolBase`   | no                                               | Isavia eAIP                                |
| Ireland              | IE   | `IE`          | `HttpEurocontrolBase`   | no                                               | AirNav Ireland eAIP                        |
| Italy                | IT   | `IT`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | ENAV Self Briefing (login-gated)           |
| Kazakhstan           | KZ   | `KZ`          | `HttpEurocontrolBase`   | no                                               | Kazaeronavigatsia (ans.kz) eAIP            |
| Kosovo               | XK   | `XK`          | `HttpEurocontrolBase`   | no                                               | CAA Kosovo (kans-ks.org) eAIP              |
| Kyrgyzstan           | KG   | `KG`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | ANSP.kg SPA portal (gated)                 |
| Latvia               | LV   | `LV`          | `HttpEurocontrolBase`   | no                                               | LGS (Latvia) eAIP                          |
| Lithuania            | LT   | `LT`          | `HttpCrawlerBase`       | via Web Unlocker                                 | ANS Lithuania (ans.lt) eAIP + VFR Manual   |
...
| New Zealand          | NZ   | `NZ`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | Airways NZ / aip.net.nz (WAF-gated)        |
...
| Portugal             | PT   | `PT`          | `HttpEurocontrolBase`   | no                                               | NAV Portugal eAIP + eVFR Manual            |
| Romania              | RO   | `RO`          | `HttpCrawlerBase`       | no (static DOCS tree; browser headers)           | AISRO (aisro.ro) static AIP                |
| Russia               | RU   | `RU`          | `HttpCrawlerBase`       | no (info-page: OurAirports list, no chart crawl) | CAICA (caica.ru) DHTML portal (gated)      |
...
| Slovenia             | SI   | `SI`          | `HttpEurocontrolBase`   | no                                               | Slovenia Control eAIP (AD 2 + AD 4)        |
| Spain                | ES   | `ES`          | `HttpCrawlerBase`       | no                                               | ENAIRE (Spain) AIP                         |
...
- **Patched build deps (contact-form SMTP) - re-check on every OpenNext / worker-mailer upgrade:** two `pnpm` patches let `worker-mailer` reach `cloudflare:sockets` through the OpenNext bundle - `patches/@opennextjs__cloudflare@1.20.1.patch` (adds `"cloudflare:sockets"` to the esbuild `external` list in `bundle-server.js` **and** `open-next/createServerBundle.js`) and `patches/worker-mailer@1.2.1.patch` (points `main` at the ESM `dist/index.mjs` so the import is static). The adapter patch is **pinned to `@opennextjs/cloudflare` 1.20.1**: an OpenNext bump usually fails to apply it, so it must be re-created and the two `external:` lines re-verified (else the contact form 502s again with `Dynamic require of "cloudflare:sockets" is not supported`). Full runbook + verification steps: `docs/opennext-cloudflare-patches.md` (PR #356).
