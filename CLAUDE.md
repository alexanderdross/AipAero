...
- **CD:** `.github/workflows/cd.yml` (self-hosted) deploys on push to `main`: applies D1 migrations, `opennextjs-cloudflare deploy`, then a self-healing **"Refresh + verify prerendered lists"** step. Auth via `CLOUDFLARE_API_TOKEN` (needs edit on Workers Scripts, R2 Storage, D1, and Workers Routes) and `CLOUDFLARE_ACCOUNT_ID` repo secrets.
- **The deploy re-seeds the airport lists EMPTY and there is NO clean way to prevent it** (investigated 14.07.2026). `opennextjs-cloudflare deploy` runs `populateCache` first, which PUTs the build's empty prerenders into R2 **unconditionally** (overwrites the filled entries); there is no partial-skip. Skipping populate entirely (`wrangler deploy`) would fix the lists but break code updates to the fully-static pages (homepage, `/llms.txt`, `/terms`, `/efb`) - they carry no revalidate tag, so they would serve the OLD cached HTML forever. Worse, TWO deployers seed empty: this CD run AND a parallel **Cloudflare Workers-Builds Git integration** (configured in the dashboard, not the repo - the reason a `sleep` was needed), so a later deploy can re-seed empty AFTER an earlier refresh. The mitigation is therefore a **verified, self-healing refresh loop** (not blind timing): each round busts the country tags (`POST /api/revalidate`) + warms all list URLs, then curls the heaviest list (DE) and counts its per-row `use href="#row-link-icon"` markers (a full render has hundreds; the empty seed has ZERO), repeating until the lists are verified full or a ~5 min budget runs out (ISR still heals within 1h). The real one-time fix would be **consolidating to a single deployer** (owner decision: disable the Workers-Builds Git integration and let CD own the deploy, or vice-versa) so the refresh can run once, immediately, without racing a second seed.
