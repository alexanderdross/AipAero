...
- **Aerodrome facts** - `src/components/airport-facts.tsx` + `src/lib/airport-facts.ts`. Embedded runways / frequencies / elevation / town / website / opening hours per ICAO, **merged** from two sources: the **OurAirports** base (public domain / CC0, imported into the D1 `airport_facts` table by `crawlers/import_ourairports.py` -> `POST /api/airport-facts`; provides coordinates, elevation, runways, frequencies, `municipality` and `home_link`) and **OpenAIP** at request time when the `OPENAIP_API_KEY` secret is set (`src/lib/openaip.ts`, cached, fail-soft; OpenAIP takes precedence per field group and is the only source of opening hours - best-effort, since the OpenAIP airport schema is not publicly documented). All server-rendered; renders nothing until the importer has run / a key is set.
- **PWA** - `src/app/manifest.ts` emits `/manifest.webmanifest` so the site is installable (e.g. on an EFB tablet).

New i18n namespaces backing these - `Weather` (incl. `decode`, `openingHours`), `Common` (`viewOnMap`, `lastUpdated`, `location`, `website`) and `TradeAero` - exist in **every** locale file; the i18n parity check (`scripts/check-i18n.mjs`) forces a newly added country to ship them too. Product/roadmap notes live in `docs/pilot-wishlist.md`.
