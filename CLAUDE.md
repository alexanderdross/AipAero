...
**Legal pages (`/terms`, `/imprint`, `/privacy`) are ROOT-level, bilingual (DE+EN) pages OUTSIDE `[locale]`** (owner decision, 18.07.2026): one clean canonical URL each (`aip.aero/terms`, not `aip.aero/uk/terms`), NOT localized into every site locale - they are legal/compliance pages, not SEO landing pages. They live at `src/app/{terms,imprint,privacy}/page.tsx`, render their own `<html>`/`<body>` + Header + `<Footer global />` via the shared `src/components/legal-shell.tsx` (which calls `setRequestLocale("uk")` so they stay STATIC / prerendered - Header/Footer read next-intl and would otherwise force dynamic rendering), and hardcode bilingual JSX (English primary `<html lang="en">`, German below in a `lang="de"` block) - so they are NOT in `routing.pathnames`, NOT in the i18n message files, and NOT in the per-country sitemap. Provider data (VAT/tax/contact) + the privacy service links live in `src/lib/legal.ts`; the Terms page still carries the `AIP_SOURCES` cc-keyed attribution (drift-guarded by `check-live-countries-coverage.mjs`, which reads `src/app/terms/page.tsx`). The old `/<locale>/terms` URLs 301-redirect to `/terms` in `middleware.ts` (regex `^/[a-z]{2}(?:/en)?/(terms|imprint|privacy)/?$`). The footer's legal links point at the fixed root paths with their existing localized labels (`Footer.{terms,imprint,privacy}.title`). Contact e-mail (`mail@dross.net`) is shown but hyperlinked to `dross.net/contact`.
